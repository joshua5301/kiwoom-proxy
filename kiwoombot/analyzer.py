...
class Analyzer(QThread):
    ...
    def _create_result_file(self, output_dir_path: str) -> None:
        """
        시작 자산과 종료 자산 그리고 총 수익률에 대한 txt 파일을 output_dir_path에 생성합니다.
        ...
        with open(output_txt_path, 'w') as file:
            ...
            # 시작 자산에 대해 분석하고 이를 기록합니다.
            file.write(f'시작 주문가능금액: {self.start_deposit}원\n')
            file.write(f'시작 보유주식: {self.start_balance}\n')
            start_asset_value = self.start_deposit
            for stock_code in self.start_balance.keys():
                start_asset_value += self.start_balance[stock_code]['보유수량'] * self.price_history[stock_code][0]['현재가']
            file.write(f'시작 자산가치: {start_asset_value}원\n\n')

            # 종료 자산에 대해 분석하고 이를 기록합니다.
            file.write(f'종료 주문가능금액 : {self.final_deposit}원\n')
            file.write(f'종료 보유주식: {self.final_balance}\n')
            final_asset_value = self.final_deposit
            for stock_code in self.final_balance.keys():
                final_asset_value += self.final_balance[stock_code]['보유수량'] * self.price_history[stock_code][-1]['현재가']
    ...
    def _draw_graph(self, output_dir_path: str, stock_code: str) -> None:
        """
        stock_code에 대한 가격 및 호가 정보 그래프를 그리고 이를 output_dir_path에 저장합니다.
    ...
    def analyze_transaction(self) -> None:
        ...
        # 거래 결과를 txt 파일로 저장합니다.
        self._create_result_file(output_dir_path)

        # 각 종목마다 ohlc 그래프를 그리고 이를 저장합니다.
        for stock_code in self.balance_history.keys():
            self._draw_graph(output_dir_path, stock_code)
