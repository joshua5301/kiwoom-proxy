...
class ClientSignalHandler(QWidget):
    ...
    def _login_request_handler(self) -> None:
        ...
        if result == 0:
            logger.debug('로그인 시도 요청에 성공하였습니다.')
        else:
            logger.error(f'!!! 로그인 시도 요청에 실패하였습니다. err_code - {result}')
    ...
    def _condition_request_handler(self) -> None:
        ...
        if is_success == 1:
            logger.debug('조건 검색식 로드 요청이 성공하였습니다.')
        else:
            logger.error(f'!!! 조건 검색식 로드 요청이 실패하였습니다. !!!')
    ...
    def _condition_search_request_handler(self, condition_name: str, condition_index: int) -> None:
        ...
        if is_success == 1:
            logger.debug('조건검색식에 부합하는 종목검색에 성공하였습니다.')
        else:
            logger.error(f'!!! 조건검색식에 부합하는 종목검색에 실패하였습니다. !!!')
    ...
    def _deposit_request_handler(self, request_name: str) -> None:
        ...
        if result == 0:
            logger.debug('주문가능금액 조회에 성공하였습니다.')
        else:
            logger.error(f'!!! 주문가능금액 조회에 실패하였습니다. err_code - {result} !!!')
    ...
    def _balance_request_handler(self, request_name: str) -> None:
        ...
        if result == 0:
            logger.debug('보유주식 조회 요청에 성공하였습니다.')
        else:
            logger.error(f'!!! 보유주식 조회 요청에 실패하였습니다. err_code - {result}!!!')
    ...
    def _order_request_handler(self, order_dict: dict[str, Any], order_name: str) -> None:
        ...
        if order_dict['구분'] == '매수':
            ...
            logger.error(f'!!! 유효하지 않은 주문 타입입니다. - {order_dict["구분"]} !!!')
        ...
        if order_dict['시장가'] is True:
            how = '03'
            if price != 0:
                logger.error('!!! 시장가 주문의 경우 가격을 0으로 설정해야 합니다. !!!')
        else:
            how = '00'
        ...
        if result == 0:
            logger.debug('정상적으로 주문이 전송되었습니다.')
        elif result == -308:
            logger.error('!!! 너무 많은 주문이 동시에 전송되어 실패하였습니다. (최대 1초에 5번) !!!')
        else:
            logger.error(f'!!! 주문 전송에 실패하였습니다. err_code - {result} !!!')
    ...
    def _register_real_time_info(self, stock_code_list: list[str], fid_list: list[str], is_add: bool) -> None:
        ...
        if result == 0:
            logger.debug('정상적으로 등록되었습니다.')
        else:
            logger.error(f'!!! 등록에 실패하였습니다. err_code - {result} !!!')
