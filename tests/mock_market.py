    ...
    def run(self):
        while True:
            ...
            for stock_code in stock_codes:
                self._price_info[stock_code]['체결시간'] = datetime.datetime.now()
...
class MockMarket:
    ...
    def request_order(self, order_dict: Dict[str, Any]) -> str:
        ...
        if order_dict['구분'] == '매수':
            ...
            if order_dict['주식코드'] not in self._balance:
                self._balance[order_dict['주식코드']] = {
                    ...
                    '매입단가': self._price_info[order_dict['주식코드']]['현재가'],
                }
            # 이미 보유한 종목일 때
            else:
                total_value = self._balance[order_dict['주식코드']]['보유수량'] * self._balance[order_dict['주식코드']]['매입가'] +\
                              order_dict['수량'] * self._price_info[order_dict['주식코드']]['현재가']
                total_amount = self._balance[order_dict['주식코드']]['보유수량'] + order_dict['수량']
                self._balance[order_dict['주식코드']]['수량'] += order_dict['수량']
                self._balance[order_dict['주식코드']]['주문가능수량'] += order_dict['수량']
        ...
        # 매도주문을 처리합니다.
            ...
            # 이미 보유한 종목일 때
                ...
                self._balance[order_dict['주식코드']]['보유수량'] -= order_dict['수량']
                self._balance[order_dict['주식코드']]['주문가능수량'] -= order_dict['수량']
            self._deposit += order_dict['수량'] * self._price_info[order_dict['주식코드']]['현재가']
            if self._balance[order_dict['주식코드']]['주문가능수량'] < 0:
                raise RuntimeError('보유 주식이 충분하지 않습니다.')
            elif self._balance[order_dict['주식코드']]['주문가능수량'] == 0:
    ...
    def get_order_info(self, order_number: str) -> Dict[str, str]:
        return 'return value is not implemented'
    ...
    def get_price_info(self, stock_code: str) -> Dict[str, Any]:
        if stock_code not in self._price_info:
            price_dict = {}
            price_dict['체결시간'] = datetime.datetime.now()
    ...
    def get_ask_bid_info(self, stock_code: str) -> Dict[str, Any]:
        if stock_code not in self._ask_bid_info:
            ask_bid_dict = {}
            ask_bid_dict['호가시간'] = datetime.datetime.now()
            ask_bid_dict['매수호가정보'] = []
            ask_bid_dict['매도호가정보'] = []
